...
// BookingPeriodPayConfig
    $(function() {
        const form = $('#BookingStartDateGroupDayPrecheckConfigForm');
        ...
        if(form.find("input[name='jsonValue']").val() != undefined) {
            ...
            if(data.startDateGroup) {
                for (const groupKey in data.startDateGroup) {
                    ...
                    for (var i = 0 ;i < groupData.openday.length; i++) {
                        $("input[name='"+groupKey+"[openday][]'][value='"+groupData.openday[i]+"']").prop("checked", true);
                    }
        ...
        $('#BookingStartDateGroupDayPrecheckConfigForm .select').on('hidden.bs.select', function (e) {
            e.stopPropagation();
