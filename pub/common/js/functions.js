...
// window.addEventListener('touchstart', fn, supportsPassive ? { passive: true } : false);
	 ...
	 * ------------------------------------------------------
	 */
	let $formReceiver = $('#formReceiver'), // 대부분의 로직 처리 아이프레임
	...
	if (!_.has(window, '_CONF')) window['_CONF'] = {};
		...
		if ($parsleys.length > 0) {
			if (!_addedMyValidator) {
				...
				window.Parsley.addValidator('code', {
					...
					validateString: function (value, _requirement) {
			...
			$parsleys.each(function (i, form) {
				...
				if (!$parsleyForm.hasClass('init-validate')) {
					...
					$parsleyForm.parsley({
						...
						let $submitButton = e.$element.data('submit');
						if ($submitButton) {
				...
				$parsleyForm.parsley().on('form:submit', function () {
					let validateCheck = true;
					$('input[data-date-range-check]').each(function(){

						let currentEl = $(this);

						if(currentEl.hasClass('required') && currentEl.val() === ""){
							currentEl.focus();
							currentEl.popover({
								content: "* <b>필수 입력 사항</b>입니다.",
								placement: "top",
								trigger: "hover",
								html: true
							}).popover("show");

							currentEl.one("dp.change", function(){
						...
						let dateEls = currentEl.siblings(`[data-date-range-check]`).addBack(); // 동일한 속성 형제 (날짜 시작 / 날짜 끝)

						if (dateEls.length !== 2) return true; // 형제가 1쌍일 때만 검증

						let startEl = dateEls.first();
						let endEl = dateEls.last();

						// 첫 번째 날짜 범위에서만 검증 수행
						if (!currentEl.is(startEl)) return true;

						let startDate = new Date(startEl.val());
						let endDate = new Date(endEl.val());

						if (!isNaN(startDate) && !isNaN(endDate)) {
							if(startDate > endDate){
								currentEl.focus();
								currentEl.popover({
									content: "<b>시작 날짜</b>는 <b>종료 날짜</b>보다 이전이어야 합니다!",
									placement: "top",
									trigger: "hover",
									html: true
								}).popover("show");
	...
	window.popup = function (passedData) {
		let popupOptions = _.extend(_defaultPopupOptions, _.pick(passedData, _popupSpecs)),
			specs = [],
		...
		let url = new URI((_.has(popupOptions, 'url') && popupOptions.url) ? popupOptions.url : _URI.pathname());
	...
	window.initPopup = function () {
		let $popups = $('a[data-popup], button[data-popup]');
		if ($popups.length > 0) {
			$popups.each(function (_i, popupEl) {
				let $popup = $(popupEl);
				if (!$popup.hasClass('init-popup')) {
					...
					$popup.on('click', function (e) {
						...
						data.url = this.tagName === 'A' ? $el.attr('href') : data.popup;
						popup(data);
					});
				}
			});
	...
	window._url = function (path) {
		return _CONF.prefix + (path.startsWith('/') ? '' : '/') + path + _CONF.suffix;
	};
	...
	window.doForm = function (data) {
		let url = '';
		if ($.type(data) === 'object') {
			let path = _.has(data, 'url') ? data.url : (new URI()).pathname(),
	...
	window.awin = function (passedData) {
		let popupOptions = _.extend(_defaultPopupOptions, _.pick(passedData, _popupSpecs));
		let path = _.has(passedData, 'url') ? passedData.url : (new URI()).pathname();
		let data = _.omit(passedData, _popupSpecs);
		let query = objectToQuery(data, true);
		let url = path + query;
		let hash = 'awin' + url.hashCode();
		let specs = [];
	...
	window.doDel = function (passedData) {
		let data = _.extend(delDefault, passedData || {}),
	...
	window.doConfirm = function (passedData) {
		let data = _.extend(confirmDefault, passedData || {}), alertOption = _.extend(confirmDefault, data.confirmProps || {});
	...
	function swalObj(obj, func) {
		...
		if (_.has(obj, 'allowOutsideClick')) {
			o.allowOutsideClick = obj.allowOutsideClick;
		}
	...
	window.alert = function (msg, target) {
		const focusTarget = target || null;
		console.log(focusTarget + ' 으로 포커스');
		Swal.fire({
			title: '',
			html: msg.replace(/\n/g, '<br />'),
			icon: 'info',
	...
	window.initBytes = function () {
		let $bytes = $('textarea[data-bytes], input[data-bytes]');
		if ($bytes.length > 0) {
			$bytes.each(function (i, el) {
				...
				if (!$byte.hasClass('init-bytes')) {
					...
					let $byteInfo = $('<div class="form-text bytes-info text-right"></div>'),
	...
	window.initDatepicker = function () {
		$('.input-group.input-daterange').each(function () {
			let $this = $(this);
			if (!$this.hasClass('input-daterange-init')) {
				$this.addClass('input-daterange-init');

				$this.datepicker({
					format: _.isUndefined($this.data('format')) ? "yyyy-mm-dd" : $this.data('format'),
					todayBtn: "linked",
					clearBtn: _.isUndefined($this.data('clear')) ? false : $this.data('clear'),
					language: "ko",
					autoclose: true,
					todayHighlight: true,
					orientation: _.isUndefined($this.data('orientation')) ? "auto" : $this.data('orientation'),
					startDate: _.isUndefined($this.data('startDate')) ? "" : $this.data('startDate'),
					endDate: _.isUndefined($this.data('endDate')) ? "" : $this.data('endDate')
				});
			}
		});
		...
		$('.input-group.date').each(function () {
			let $this = $(this);
			if (!$this.hasClass('input-date-init')) {
				$this.addClass('input-date-init');
				$this.datepicker({
					format: _.isUndefined($this.data('format')) ? "yyyy-mm-dd" : $this.data('format'),
					todayBtn: "linked",
					clearBtn: _.isUndefined($this.data('clear')) ? false : $this.data('clear'),
	...
	window.initDateTimePicker = function () {
		$('.intput-datetime').each(function () {
			console.log($(this));
			const _this = $(this), _data = _this.data();
			if (!_this.hasClass('intput-datetime-init')) {
				_this.addClass('intput-datetime-init');

				// 기본 설정 복사 후 data-format 적용
				let options = _.merge(_.clone(_datetimeDefaultOption), _data.dp || {});
				if (_data.format) {
				...
				if (_.has(_data, 'enddate')) {
					_this.on('dp.change', function (e) {
						$(_data.enddate).data("DateTimePicker").minDate(e.date);
					});
				}
				if (_.has(_data, 'startdate')) {
					_this.on('dp.change', function (e) {
						$(_data.startdate).data("DateTimePicker").maxDate(e.date);
					});
				}
			}
		});
	...
	/**
	 * tbody > tr 정렬 가능한 테이블 초기화
	 */
	window.initSortableTable = function () {
		let $sortTables = $('.table-sort');
		if ($sortTables.length > 0) {
			$sortTables.each(function (i, sortTable) {
				let $sortTable = $(sortTable);
				if (!$sortTable.hasClass('init-sort-table')) {
					$sortTable.addClass('init-sort-table');
					let $thead = $sortTable.find('thead > tr'),
						$tbody = $sortTable.find('tbody');
					$thead.prepend('<th><i class="fal fa-sort"></i></th>');
					$tbody.find('tr').each(function (i, tr) {
						$(tr).prepend('<td><i class="fal fa-bars sortable-handle" style="cursor:move;"></i></td>');
					});
					let sortOptions = {
						items: '> tbody > tr',
						...
						update: function (e, ui) {
							...
							let $target = $(e.target), // table element
								key = $target.data('pk'),
								keys = $target.data('pks'),
								addParams = $target.data('param');
							if (addParams) {
								params = _.extend(params, addParams);
							}
							if (key) {
								let values = [];
								$target.find('tbody tr').each(function (i, tr) {
									values.push($(tr).data(key));
								});
								params[key + '[]'] = values;
							} else {
					...
					_.extend(sortOptions, $sortTable.data() || {});
					// console.log(sortOptions);
					$sortTable.sortable(sortOptions);
				}
			});
	...
	/**
	 * 체크 가능한 테이블
	 * <table id="theTable" class="table-check" data-button="체크시 활성화할 버튼 Selector">
	 * <tr class="has-data" data-id="id"> 의 data-[key]="value" 가 버튼 클릭시 객체로 전달됨
	 * $('#theTable').on('btn-click', function(data){}); 로 버튼 클릭 이벤트 받을 수 있음
	 */
	window.initCheckableTable = function () {
		let $checkTables = $('.table-check');
		if ($checkTables.length > 0) {
			let checkEnableButtons = function () {
				let $checkedInputs = $checkTables.find('tbody > tr > td.check-container > input.checker:checked');
				$($checkTables.data('button')).prop('disabled', $checkedInputs.length <= 0);
			};

			$checkTables.each(function (i, checkTable) {
				let $checkTable = $(checkTable);
				if (!$checkTable.hasClass('init-check-table')) {
					$checkTable.addClass('init-check-table');
					let $thead = $checkTable.find('thead > tr'),
						$tbody = $checkTable.find('tbody'),
						$trs = $tbody.find('tr.has-data'),
						$checkTablebutton = $($checkTable.data('button'));
					if ($checkTablebutton.length > 0) {
						$checkTablebutton.prop('disabled', true);
						$checkTablebutton.on('click', function () {
							let $thisButton = $(this),
								checkedDatas = [];
							$tbody.find('tr > td.check-container > input.checker:checked').each(function () {
								checkedDatas.push($(this).closest('tr').data());
							});
							$checkTable.trigger('btn-click', [checkedDatas, $thisButton]);
						});
					}
					if ($trs.length > 0) {
						$thead.prepend('<th class="check-trigger-container"><input type="checkbox" class="check-trigger" /></th>');
						$trs.each(function (i, tr) {
							if (!$(tr).hasClass('no-checkable')) {
								$(tr).prepend('<td class="check-container"><input type="checkbox" class="checker" name="' + $checkTable.data('name') + '" value="' + $(tr).data('value') + '" /></td>');
							} else {
								$(tr).prepend('<td></td>');
							}
						});
						$thead.find('th.check-trigger-container > input.check-trigger').on('click', function () {
							$trs.find('td.check-container > input.checker').prop('checked', $(this).is(':checked'));
							checkEnableButtons();
						});
						$trs.find('td.check-container > input.checker').on('click', checkEnableButtons);
					}
				}
			});
		}
	...
	/**
	 * bootstrap-select 초기화 https://developer.snapappointments.com/bootstrap-select/examples/
	 */
	// $.fn.selectpicker.Constructor.DEFAULTS
	if ($.type($.fn.selectpicker) == 'function') {
		$.fn.selectpicker.Constructor.DEFAULTS.deselectAllText = '선택 해제';
		$.fn.selectpicker.Constructor.DEFAULTS.noneSelectedText = '선택 안함';
		$.fn.selectpicker.Constructor.DEFAULTS.selectAllText = '모두 선택';
		$.fn.selectpicker.Constructor.BootstrapVersion = '4';
	}
	let _init_select_defaults = {
		container: 'body',
		showSubtext: true,
	...
	window.initSelect = function () {
		$('select.select').each(function (i, select) {
			let $select = $(select);
			if (!$select.hasClass('init-select')) {
				$select.addClass('init-select');
				$select.selectpicker(_.merge(_.clone(_init_select_defaults), $select.data()));
			}
		});
	};
	 ...
	 * Label Picker
	 */
	window.initLabelPicker = function () {
		let $labelPickers = $('.label-picker');
		$labelPickers.each(function (i, picker) {
			let $picker = $(picker);
			if (!$picker.hasClass('init-label-picker')) {
				$picker.addClass('init-label-picker');
				let data = $picker.data(),
					$labels = $picker.find('label'),
					$inputs = $labels.find('input[type=radio]'),
					$checkers = $labels.find('.checker');
				$checkers.html('<i class="fa fa-fw"></i>');

				if (data.value) {
					$picker.find('input[value="' + data.value + '"]').prop('checked', true);
					$picker.find('input[value="' + data.value + '"]').next().html('<i class="fa fa-fw fa-check"></i>');
				}
				...
				$labels.on('click', function () {
					...
					$(this).find('.checker').html('<i class="fa fa-check"></i>');
				});
			}
		});
	};
	...
	window._initDataView = function () {
		let dataViews = $('[data-view]');
		dataViews.each(function (_, el) {
			let $el = $(el);
			if (!$el.hasClass('init-data-view')) {
				$el.addClass('init-data-view');
	...
	function initViewModal(option) {
		let $modal = $($('#viewModalTemplate').html());
		$modal.find('.modal-title').html(option.title || '<i class="fal fa-check"></i> 상세');
		return $modal;
	}
	...
	$(function () {
		$('[data-add]').on('click', function (e) {
			e.preventDefault();
			e.stopPropagation();
			addwin($(this).data('add'));
		});
		_initDataView();
		$('[data-view-modal]').on('click', function (e) {
			e.preventDefault();
			e.stopPropagation();
			let data = $(this).data('viewModal');
			let option = _.extend({
				act: 'index',
				cmd: 'view',
				_out: 'body'
			}, data || {})
			let $modal = initViewModal(option);
			let path = (new URI()).pathname();
			let query = objectToQuery(option, true);
			let url = path + query;
			...
			$modal.on('show.bs.modal', function () {
				$modal.find('.modal-body').load(url, initUI);
			});
			$modal.modal();
		});
		...
		$('[data-flag]').on('click', function (e) {
			...
			doForm(_.extend(defaults, $(this).data('flag') || {}));
		}).css('cursor', 'pointer');

		$('[data-setvalue]').on('click', function (e) {
			e.preventDefault();
			e.stopPropagation();
			let defaults = {
				act: 'index',
			...
			doForm(_.extend(defaults, $(this).data('setvalue') || {}));
		}).css('cursor', 'pointer');
		...
		$('[data-confirm]').on('click', function(e) {
			e.preventDefault();
			e.stopPropagation();
			const msg = $(this).data('msg') || '계속하시겠습니까 ?';
			const act = $(this).data('confirm');
			const data = _.omit($(this).data(), ['confirm', 'msg']);
			data.act = act;
			showConfirm('확인', msg, function(ok) {
				if(ok) {
					doForm(data);
				}
			});
	...
	window.loadScript = function (src) {
		return new Promise(function (resolve, reject) {
			...
			script.onerror = () => reject(new Error(`${src} 로드 중 오류`));
